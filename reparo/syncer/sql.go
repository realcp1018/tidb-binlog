...
type sqlSyncer struct {
	destSqlFile io.ReadWriteCloser
}

func newSqlSyncer(sqlFile string) (*sqlSyncer, error) {
	if sqlFile == "" {
		return &sqlSyncer{destSqlFile: os.Stdout}, nil
	}
	file, err := os.OpenFile(sqlFile, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0644)
	if err != nil {
		return nil, err
	}
	return &sqlSyncer{destSqlFile: file}, nil
}

func (s *sqlSyncer) Sync(pbBinlog *pb.Binlog, cb func(binlog *pb.Binlog)) error {
	switch pbBinlog.Tp {
	case pb.BinlogType_DDL:
		if err := s.writeDDL(pbBinlog); err != nil {
			return err
		}
...
func (s *sqlSyncer) writeDDL(binlog *pb.Binlog) error {
...
// writeDMLEvent: writeInsertEvent, writeDeleteEvent, writeUpdateEvent
func (s *sqlSyncer) writeDMLEvent(event *pb.Event) error {
	switch event.GetTp() {
...
func (s *sqlSyncer) writeInsertEvent(schema string, table string, row [][]byte) error {
...
func (s *sqlSyncer) writeDeleteEvent(schema string, table string, row [][]byte) error {
...
func (s *sqlSyncer) writeUpdateEvent(schema string, table string, row [][]byte) error {
	...
	for i, c := range row {
		...
		sBefore := formatWithNull(formatValueToSqlString(val, col.Tp[0]), true)
		sAfter := formatWithNull(formatValueToSqlString(changedVal, col.Tp[0]), false)
		if i == len(row)-1 {
...
// utils: writeTranStart, writeTranStop, writeLineSep
func (s *sqlSyncer) writeLineSep() {
	_, _ = s.destSqlFile.Write([]byte("\n"))
}

func (s *sqlSyncer) writeTxnStart(commitTS int64) {
	_, _ = s.destSqlFile.Write([]byte(fmt.Sprintf("Start Transaction; # %d\n", commitTS)))
}

func (s *sqlSyncer) writeTxnStop(commitTS int64) {
	_, _ = s.destSqlFile.Write([]byte(fmt.Sprintf("Commit; # %d\n", commitTS)))
}

func formatWithNull(s string, isWhereCondition bool) string {
	if s == "null" && isWhereCondition {
		return " IS NULL"
	} else {
