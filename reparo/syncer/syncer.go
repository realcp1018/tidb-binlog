...
func New(name string, destSqlFile string, cfg *DBConfig, worker int, batchSize int, safemode bool) (Syncer, error) {
	switch name {
	case "mysql":
	...
	case "sql":
		return newSqlSyncer(destSqlFile)
	case "flashback":
		return newFlashbackSyncer(destSqlFile)
	case "memory":
		return newMemSyncer()
	}
